...
class DebrisAnalyzer:
    ...
    def _determine_feasibility(self, size: float, material: str, melting_feasibility: Dict) -> bool:
        """Determine overall feasibility for capture and melting"""
        # Size constraints
        size_feasible = self.min_feasible_size <= size <= self.max_feasible_size
        
        # Melting feasibility
        melting_feasible = melting_feasibility['feasible']
        
        # Material feasibility (some materials are harder to handle)
        material_feasible = material not in ['unknown']
        
        return size_feasible and melting_feasible and material_feasible
